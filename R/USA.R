...
extract_content <- function(url = ...){
  content <- url %>% xml2::read_html() %>% 
    rvest::html_nodes(css ="#News_Body_Txt_A p") %>% rvest::html_text() %>% str_remove_all(BLANK) %>% 
    str_subset("") %>% str_c(collapse = TRUE)
  return(content)
}
html <- xml2::read_html("http://www.china-embassy.org/chn/lszj/zytz/default.htm")
...
page <- tibble(Title = rvest::html_nodes(html, css = "#docMore a") %>% rvest::html_text() %>% 
  ...
  mutate(url = url %>% 
           str_replace(START %R% DOT, "http://www.china-embassy.org/chn/lszj/zytz")) %>% 
